//
//  OnboardingV3StageProgressHeader.swift
//  Fitness Coach
//
//  Forma — Stage-based progress header for tap-first onboarding v3.
//

import SwiftUI

struct OnboardingV3StageProgressHeader: View {
    let currentStep: OnboardingV3Step

    private var currentStage: OnboardingV3Stage {
        currentStep.stage
    }
    ...
    private var stageSegmentBar: some View {
        HStack(spacing: 4) {
            ForEach(OnboardingV3Stage.allCases) { stage in
                Capsule()
    ...
    private func segmentFill(for stage: OnboardingV3Stage) -> Color {
...
#Preview {
    OnboardingV3StageProgressHeader(currentStep: .bodyBasics)
        .padding()
