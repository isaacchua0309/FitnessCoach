//
//  OnboardingV4PageShell.swift
//  Fitness Coach
//
//  Forma — Reusable page shell for v4 onboarding screens.
//

import SwiftUI

struct OnboardingV4PageShell<Content: View>: View {
    var currentStep: OnboardingV4Step?
    var title: String?
    var subtitle: String?
    ...
    private var bodyStack: some View {
        VStack(alignment: .leading, spacing: OnboardingLayout.compactSectionSpacing) {
            if showsProgressHeader, let currentStep {
                OnboardingV4StageProgressHeader(currentStep: currentStep)
            ...
            VStack(alignment: .leading, spacing: OnboardingLayout.progressTitleSpacing) {
                ...
                if let helperText {
                    Text(helperText)
                        ...
                        .fixedSize(horizontal: false, vertical: true)
                        .accessibilityLabel(
                            "\(FormaProductCopy.Onboarding.V4.Components.helperAccessibilityPrefix). \(helperText)"
                        )
                }
            }
