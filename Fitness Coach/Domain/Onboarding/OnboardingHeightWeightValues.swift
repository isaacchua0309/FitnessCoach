//
//  OnboardingHeightWeightValues.swift
//  Fitness Coach
//
//  Forma — Canonical metric storage and imperial conversion for height/weight.
//

import Foundation

enum OnboardingHeightWeightValues {
    ...
    static func applyDefaultsIfNeeded(to formState: inout OnboardingFormState) {
        if formState.heightCmText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            formState.heightCmText = formatStoredMetric(OnboardingPickerDefaults.defaultHeightCm)
        }
        if formState.currentWeightKgText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            formState.currentWeightKgText = formatStoredMetric(OnboardingPickerDefaults.defaultWeightKg)
        }
    }
    ...
    static func resolvedHeightCm(from formState: OnboardingFormState) -> Double {
        ...
        return OnboardingPickerDefaults.defaultHeightCm
    }
    ...
    static func resolvedWeightKg(from formState: OnboardingFormState) -> Double {
        ...
        return OnboardingPickerDefaults.defaultWeightKg
    }
    ...
    static func setImperialHeight(feet: Int, inches: Int, in formState: inout OnboardingFormState) {
        ...
        let clamped = min(
            max(totalInches, Int(OnboardingPickerDefaults.imperialHeightInchesRange.lowerBound.rounded())),
            Int(OnboardingPickerDefaults.imperialHeightInchesRange.upperBound.rounded())
        )
    ...
    static func validateWeightKg(_ value: Double) throws {
        guard OnboardingPickerDefaults.metricWeightKgRange.contains(value) else {
            throw OnboardingFormError.invalid(
                FormaProductCopy.Onboarding.Flow.Validation.weightOutOfRange
            )
        }
    }
