//
//  OnboardingV4ComponentsTests.swift
//  Fitness CoachTests
//
//  Forma — Unit tests for v4 onboarding reusable components.
//

import XCTest
@testable import Fitness_Coach

final class OnboardingV4ComponentsTests: XCTestCase {

    // MARK: - Ruler math

    func testRulerMathBuildValuesUsesStep() {
        let values = OnboardingV4RulerMath.buildValues(in: 70...72, step: 0.5)
        XCTAssertEqual(values, [70, 70.5, 71, 71.5, 72])
    }

    func testRulerMathIndexFindsNearestValue() {
        let values = OnboardingV4RulerMath.buildValues(in: 70...72, step: 0.5)
        XCTAssertEqual(OnboardingV4RulerMath.index(for: 71.2, in: values), 2)
        XCTAssertEqual(OnboardingV4RulerMath.index(for: 71.8, in: values), 4)
    }

    func testRulerMathSnapValue() {
        let values = OnboardingV4RulerMath.buildValues(in: 70...72, step: 0.5)
        XCTAssertEqual(OnboardingV4RulerMath.snapValue(71.24, in: values), 71)
        XCTAssertEqual(OnboardingV4RulerMath.snapValue(71.26, in: values), 71.5)
    }

    func testRulerMathClampedIndex() {
        XCTAssertEqual(OnboardingV4RulerMath.clampedIndex(-3, count: 5), 0)
        XCTAssertEqual(OnboardingV4RulerMath.clampedIndex(99, count: 5), 4)
        XCTAssertEqual(OnboardingV4RulerMath.clampedIndex(2, count: 5), 2)
    }

    func testRulerMathAccessibilityLabelIncludesUnit() {
        let label = OnboardingV4RulerMath.accessibilityValueLabel(
            value: 72,
            unitLabel: "kg",
            formatter: { "\(Int($0))" }
        )
    ...
    func testBirthdayWheelFactoryYearRangeRespectsAgeBounds() {
        ...
        let columns = OnboardingV4BirthdayWheelFactory.columns(referenceDate: reference, calendar: calendar)
    ...
    func testBirthdayWheelFactoryComposesValidDate() {
        let calendar = Calendar(identifier: .gregorian)
        let date = OnboardingV4BirthdayWheelFactory.birthDate(month: 3, day: 15, year: 1990, calendar: calendar)
    ...
    func testIntroProofBulletsMapFromProductCopy() {
        let bullets = OnboardingV4FeatureBullet.introProofDefaults
        let copyBullets = FormaProductCopy.Onboarding.V4.IntroProofFeatures.bullets
    ...
    func testAlmostThereBulletsMapFromProductCopy() {
        ...
        let copyBullets = FormaProductCopy.Onboarding.V4.AlmostThereFeatures.bullets
    ...
    func testWeightMaintenanceProofModelUsesCopyNotHardcodedClaims() {
        let model = OnboardingV4WeightMaintenanceProofModel.introDefault
    ...
    func testComparisonBarProofModelUsesCopyNotHardcodedClaims() {
        let model = OnboardingV4ComparisonBarProofModel.introDefault
    ...
    func testTrajectoryComparisonModelUsesIntroProofCopy() {
        let model = OnboardingV4WeightTrajectoryComparisonModel.introProofDefault
        let intro = FormaProductCopy.Onboarding.V4.IntroProof.self
        let trajectory = FormaProductCopy.Onboarding.V4.Proof.TrajectoryComparison.self
    ...
    func testIntroProofStepCopyMatchesProductConstants() {
        let step = OnboardingV4Step.introProof
        let copy = FormaProductCopy.Onboarding.V4.IntroProof.self
