...
final class AccountSyncOutboxStoreTests: XCTestCase {
    ...
    func testMarkInFlightSucceededFailedAndCancelRequireOwnerUID() async throws {
        ...
        let dueCancellableMutations = try await outbox.fetchDueMutations(ownerUID: ownerUID, limit: 1, now: referenceDate)
        let cancellableID = try XCTUnwrap(dueCancellableMutations.first?.id)
    ...
    func testSucceededMutationsCanBePruned() async throws {
        ...
        let dueOwnerMutations = try await remoteOutbox.fetchDueMutations(ownerUID: ownerUID, limit: 1, now: referenceDate)
        let ownerMutationID = try XCTUnwrap(dueOwnerMutations.first?.id)
        ...
        let dueOtherMutations = try await remoteOutbox.fetchDueMutations(ownerUID: otherOwnerUID, limit: 1, now: referenceDate)
        let otherMutationID = try XCTUnwrap(dueOtherMutations.first?.id)
