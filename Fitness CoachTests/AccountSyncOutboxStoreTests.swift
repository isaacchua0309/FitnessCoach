...
final class AccountSyncOutboxStoreTests: XCTestCase {
    ...
    func testMarkInFlightSucceededFailedAndCancelRequireOwnerUID() async throws {
        ...
        let cancelDueMutations = try await outbox.fetchDueMutations(
            ownerUID: ownerUID,
            limit: 1,
            now: referenceDate
        )
        let cancellableID = try XCTUnwrap(cancelDueMutations.first?.id)
    ...
    func testSucceededMutationsCanBePruned() async throws {
        ...
        let ownerDueMutations = try await remoteOutbox.fetchDueMutations(
            ownerUID: ownerUID,
            limit: 1,
            now: referenceDate
        )
        ...
        let otherDueMutations = try await remoteOutbox.fetchDueMutations(
            ownerUID: otherOwnerUID,
            limit: 1,
            now: referenceDate
        )
