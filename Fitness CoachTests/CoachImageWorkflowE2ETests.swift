...
XCTAssertTrue(stagedPhoto4)
    ...
    func testSendCurrentMessageDoesNotAbortWhenProcessingLockAcquired() async throws {
        ...
        let stagedPhoto5 = await CoachImageWorkflowTestSupport.stageTestMealPhoto(on: model, jpeg: jpeg, source: .library)
XCTAssertTrue(stagedPhoto5)
    ...
    func testImageOnlySendCreatesUserBubbleBackendPayloadAndMealDraft() async throws {
        ...
        let stagedPhoto6 = await CoachImageWorkflowTestSupport.stageTestMealPhoto(on: model, jpeg: jpeg, source: .library)
...
XCTAssertTrue(stagedPhoto7)
    ...
    func testTextAndImageSendShowsBothInBubbleAndBackend() async throws {
        ...
        let stagedPhoto8 = await CoachImageWorkflowTestSupport.stageTestMealPhoto(on: model, jpeg: jpeg, source: .library)
...
XCTAssertTrue(stagedPhoto9)
    ...
    func testRetryReusesSameImageAndUpdatesSessionResults() async throws {
        ...
        let stagedPhoto10 = await CoachImageWorkflowTestSupport.stageTestMealPhoto(on: model, jpeg: jpeg, source: .library)
...
XCTAssertTrue(stagedPhoto12)
    ...
    func testRapidAddRemoveThenSendImageOnlyUsesLatestAttachment() async throws {
        ...
        let stagedPhoto13 = await CoachImageWorkflowTestSupport.stageTestMealPhoto(on: model, jpeg: first, source: .library)
XCTAssertTrue(stagedPhoto13)
        model.removeStagedMealPhoto()
        let stagedPhoto14 = await CoachImageWorkflowTestSupport.stageTestMealPhoto(on: model, jpeg: second, source: .camera)
XCTAssertTrue(stagedPhoto14)
        await model.sendCurrentMessage()
