...
export function coachContextV2Rules(): string {
  return [
    ...
    "8. Use context.recentMealsStructured for meal-specific references (names, macros, linkedEntryId).",
    "9. Use context.training, context.healthIntelligence, and context.today when giving meal or recovery advice.",
    "10. Never claim HealthKit or synced health data exists when context.missingData marks it unavailable.",
    "11. Do not diagnose medical conditions or prescribe medical treatment.",
    "12. Do not mutate app state. Return intents, drafts, estimates, or coaching text only.",
  ].join("\n");
}
...
export function classifyCoachIntentPromptRules(): string {
  return [
    ...
    "- Do not copy nutrition values from context.recentChatMessages or assistant text.",
